...
globalThis.jsConnect = async function(appId, args, dartOnMessage) {
    ...
    if (typeof showLoadingOverlay === "function") {
        ...
        if (typeof setLoadingStatus === "function") {
            setLoadingStatus("Starting Python runtime...");
        }
    ...
    app.worker.onmessage = (event) => {
        if (event.data && typeof event.data === "object" && event.data.type === "pylog") {
            const level = event.data.level === "stderr" ? "error" : "log";
            console[level](`[pyodide ${event.data.level}] ${event.data.message}`);
            return;
        }
        if (event.data && typeof event.data === "object" && event.data.type === "progress") {
            if (typeof setLoadingStatus === "function") {
                setLoadingStatus(event.data.message || "Loading...");
            }
            if (typeof setLoadingProgress === "function") {
