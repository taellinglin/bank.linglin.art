...
class BlockchainDaemon:
    ...
    def validate_chain(self, chain: List[Dict]) -> bool:
        ...
        return True
    def mine_with_guaranteed_cleanup(self, miner_address: str, difficulty: int = 2) -> Dict:
        """Mining with guaranteed mempool cleanup"""
        try:
            # Run before mining
            self.comprehensive_mempool_cleanup()
            
            # Mine normally
            result = self.mine_pending_transactions(miner_address, difficulty)
            
            # Run cleanup again after mining
            if result:
                self.comprehensive_mempool_cleanup()
                
                # Double-check that mined transactions are removed
                mined_transactions = result.get("transactions", [])
                for tx in mined_transactions:
                    if self.is_transaction_mined(tx):
                        self.logger.info(f"✅ Verified transaction mined: {tx.get('hash', 'no-hash')[:16]}...")
                    else:
                        self.logger.error(f"❌ Transaction not properly mined: {tx.get('hash', 'no-hash')[:16]}...")
            
            return result
            
        except Exception as e:
            self.logger.error(f"Mining with cleanup error: {e}")
            return None
    ...
    def bootstrap_blockchain(self) -> bool:
        ...
        success = self.create_genesis_block()
                ...
                try:
                    ...
                    if iteration % 1 == 0:
                        ...
                        if status["available_transactions"]["total"] > 0:
                            result = self.mine_with_guaranteed_cleanup(miner_address=miner_address)
